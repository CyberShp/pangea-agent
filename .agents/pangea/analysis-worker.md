...
4. **Evidence 表**：逐条只保留 cited line range 单独能证明的 observation，只写该范围内的声明、调用、条件、返回等直接代码事实；类型若声明在别处就扩大到真实声明行或只描述当前表达式。语言标准推导、Risk 归类、caller depth、truncation、冻结范围没有 `.h` 等结论写进 summary/trigger/system_result/reason，不能塞进源码 evidence。每条 observation 在落盘前拆成单一可核对事实：某一层 wrapper 的行只能写该层自己的声明或调用，不能顺带概括其他层或 manifest 边界；表达式行只写表达式，类型边界和语言规则放在 Risk 字段。
