...
Worker 结束前可调用 `check-result-json --task`。该命令只读取 task 指向的结果，确认 JSON 可解析，并以非阻断 `advisories` 提示 schema、内部编号、声明链接和证据路径问题；不写文件、不改变 action / Run 状态、不判断语义，也不能代替 settle。任何内容修正仍由当前 Agent 自己完成。
