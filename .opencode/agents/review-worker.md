...
- **Evidence isolation**：逐条只保留 cited line range 单独能证明的 observation。例如表达式所在行不能同时证明声明在别处的类型，源码行也不能单独证明语言标准结论或 Risk 分类；扩大到真实声明行，或把类型/语言规则推导写入 summary/trigger/system_result。`layer08.c` 只能证明它自己的声明/调用；caller depth、truncation、冻结范围没有 `.h` 等事实只能写 summary/required_check/conclusion，不能写进该源码 evidence。
- **结构化审计账本**：`review_contract_version=2.0` 时，task 的 `required_analysis_audits` 是 Workflow 从 validated Analysis 生成的必审身份清单。`analysis_audit_decisions[].audit_id` 集合必须与它完全相等且不重复。逐项真正执行 `check` 并填写该对象/字段独有的 `conclusion`：确认无误填 `accepted`、`finding_keys=[]`，并写出成立的 exact 字段值与冻结依据；发现错误填 `finding`，写出具体不一致，并引用 retained 的 `confirmed|unresolved` Independent finding 或本 Comparison `findings[]` 的真实 `finding_key`。不得用同一句泛化结论批量填写多个 audit，不得省略任何审计目标。
