...
若 Analysis 对源码事实本身就判断错误，例如把实际失败返回 0 说成返回一个大整数，使用 `incorrect_conclusion`；若源码事实和翻译方向没有明确错误，只是缺少一个必要可观察验证点，使用 `test_oracle`。Reviewer 只写 finding，由原 Analysis worker 在 Closure 修正 Scenario/TestCase。
