...
- **Scenario/Risk**：先只看每个 Scenario 的 `observed_fields.actions`，逐项二选一：它本身是在陈述具体内部操作/predicate/outcome，还是仍在询问、等待确认如何操作。后一类一律不能让 `trigger_actions|developer_confirm_content` accepted；必须建立精确 finding 或删除 Scenario，不能用 readiness=`developer_confirm`、title、preconditions、evidence 或“如实表达待确认”作为通过理由。再检查 Risk 的普通构建观测是否误写成必然返回，以及 sanitizer 是否被当成 exclusion。Analysis 已有等价 Risk 时可以 dismiss Independent Risk finding，但关联对象仍有错误时必须按下方 taxonomy 另建精确 finding。
