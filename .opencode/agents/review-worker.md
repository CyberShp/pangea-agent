...
- **分批审计**：当 task 提供 `audit_batch_count>1` 时，只裁决当前 `audit_batch_index` 的 `required_analysis_audits`，不得声称未出现在本批的 audit 已完成，也不得复制或猜测其他批次的 decision。首批（index=0）填写全部 `independent_finding_decisions`；后续批次只填写本批 Analysis audit，若引用 Independent finding 可复用其已冻结 finding_key。每批都写入当前 task 的唯一 `result_path`，结束后由同一 Reviewer Session 按下一 action 继续；不得创建新的 Reviewer 或自行合并结果。
...
- **Scenario action litmus**：`actions` 以“待确认”“待开发确认”“需要确认”开头，或仍在询问如何构造/调用时，只是待办问题，不是已承载 predicate/outcome；`scenario/trigger_actions` 与 `developer_confirm_content` 必须填 `finding` 或要求删除 Scenario。入口、制造方式和产品 Oracle 可以待确认，但不能把该前缀的 action 当作已承载 Risk/Branch trigger。
