...
- 风险：先核对 `upstream_semantics` 中的入口可达性、调用方限制/补救、规格或高层 API 定义、已有测试实际覆盖；已被上游定义为预期行为的结论不得保留为风险。但“接口允许返回失败”不等于“失败后的状态必然安全”：若底层实现可能先产生部分副作用再失败，必须核对重试、关闭和恢复是否仍可执行；清理语句位于 `assert(false)` 之后时，还要分别判断 Debug 与 Release 路径。再检查复现条件、系统结果、外部观测、排除条件、严重度、置信度和证据是否一致。
- 用例：必须关联真实风险，步骤和预期结果一一对应，观测与清理可执行；故障注入只能制造触发条件，不能预先制造待验证结果；产品已释放对象后不得继续使用失效句柄，必须改用新业务操作、资源计数、日志或新连接观测；预期结果不得使用“A 或 B”。不得加入优先级、安全专项、SFMEA、代码建议或实现质量评价。
