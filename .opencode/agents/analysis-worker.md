...
6. 写入 `test_cases` 前调用 `product-blackbox-test-case` Skill，并逐项执行 `test_case_generation.md` 的“从源码发现转换为测试语义”步骤；转换完成后才能写测试步骤和预期结果，不得直接把源码或认证报文改写成用例。该 Skill 只改善最终用例表达，不新增状态、门禁、审计或返工。
   每个步骤必须有一个同位置的预期结果，`steps` 与 `expected_results` 数量完全一致；并逐条确认用例的触发条件和观测现象确实能验证其 `linked_risk_ids`。删除、注销或关闭类系统调用失败时，前置条件必须选定一个真实可制造的失败原因和对应对象状态；泛称“底层操作返回失败”或只让桩返回错误不能生成可执行用例。前置条件已限定 Debug 或 Release 构建时，预期不得混入另一构建，也不得使用“可能”。
