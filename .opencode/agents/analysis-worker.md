...
2. **Scenario 保留表**：对每个准备保留的 Scenario 单独从其 `actions` 原文抄出两段：具体 predicate/trigger 与对应源码 outcome；两段都必须实际存在于这个 Scenario 自己的 `actions` 数组中，不能从 title、preconditions、external_oracles 或另一个 Scenario 借用。再把 action 二选一标为“已经陈述具体内部操作/条件”或“仍在询问、等待确认如何操作”；只有前一类可以进入 `actions`，后一类属于缺口，只能写在 `business_entry|preconditions|reason`。受支持入口未确认时，action 只能陈述冻结源码内部的条件、操作与 outcome，不得声称“通过产品入口/公开 API”执行。任一段无法抄出，或 action 仍是待回答问题，就删除 Scenario，而不是用 readiness=`developer_confirm` 保留占位内容。Risk-linked Scenario 还必须在自己的 `external_oracles` 中写出冻结证据允许的条件性观测；只有“普通构建无稳定 Oracle/结果不可依赖”时，移除 Risk 链接或 Scenario。删除 Scenario 时同步清理 Branch/Coverage/Risk 引用。
3. **UB 与 trigger 表**：source-proven UB Risk 的 `trigger` 只写冻结源码证明的精确内部条件和对应源码操作；未确认受支持入口时，不得把私有 wrapper 或待确认测试桩写成已经可用的“测试入口/业务入口”，入口与制造方式留在 disposition/reason。未冻结 ABI、目标编译器或构建契约时保留 `INT_MAX` / 对应 `TYPE_MAX` 符号，不得追加“32 位”等具体位宽或十进制边界。普通构建必须明确任何返回值、终止状态或其他表现都不是稳定产品 Oracle；可以列举“不返回、异常终止、表面返回任意值”等可能后果，但不得写成必然结果，也不得把数学上的 `INT_MAX + 1` 称为该 `int` 运算可比较的“预期值”。未冻结具体编译器、版本、优化与运行时契约时，也不得声称某种返回、终止或非返回表现“通常、一般、常见”发生。sanitizer 只能作为明确构建前提下的条件性观测，不能写进 `exclusion_condition`；即使同一句随后声明它“只改变观测、不消除风险”，也不能把它列为排除条件的并列候选。
