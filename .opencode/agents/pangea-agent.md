...
- analysis-worker 初次返回 `STAGE checkpoint` 后，记录其 task_id 并恢复同一会话，消息只写“继续同一 task 的 risks 阶段”；返回 `STAGE risks` 后再次恢复同一会话，消息只写“继续同一 task 的 tests 阶段并校验”。这两个计划内阶段返回不是提交失败、schema 修正或正式返工；只有最终 `validate-worker-result=PASS` 才推进 graph。
...
- Worker 在应返回 `STAGE checkpoint`、`STAGE risks` 或最终 PASS 时返回空 task_result，且对应阶段没有写入结果文件，才按提交失败处理：优先恢复同一会话；连续两次空返回才允许替换 worker。替代 worker 仍处理同一 task、同一 attempt、同一结果路径，并从 summary 标记的未完成阶段继续。
