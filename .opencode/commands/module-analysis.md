...
5. 当前 Agent 回合通过自己 task 的提交检查后，主 Agent 立即按 `after_completion=resume_run` 恢复同一 `run_id` 和 `data_root`，直到生成 `report.md` 和 `report.html`。OpenCode 恢复原会话或 DSH 切换回历史会话时沿用该会话原有 `run_id`；只有用户在新会话明确指定历史 Run 时才恢复历史 Run。
