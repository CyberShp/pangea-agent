...
本仓库以冻结的 `codetalks-skill 1.4.0` Run 作为唯一分析流程。主会话只创建 Run、冻结输入并读取真实状态；不自行分析源码、不编排旧 action、不填写语义结果。
...
- Desktop/DSH 只启动一个 Skill Run；Skill 自己按当前冻结 manifest（新模块分析为五阶段） 管理 Producer、独立 Judge、正式交付和门禁。
