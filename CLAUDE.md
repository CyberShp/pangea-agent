...
Python 负责确定性解析、状态、契约和报告，不判断测试用例或 Reviewer 的语义结论。Review 由同一 Reviewer 完成 `independent_review` 和 `comparison_review`。定向补齐以 `continue_agent` 续接对应单元首轮 analysis worker；Graph 预先复制原结果，worker 只修改 closure `result_path`。
