...
2. **Scenario 保留表**：先把每个准备写入的 action 二选一标为“已经陈述具体内部操作/条件”或“仍在询问、等待确认如何操作”。只有前一类可以进入 `actions`；后一类属于缺口，只能写在 `business_entry|preconditions|reason`，不能因为 readiness=`developer_confirm` 而保留为 action。每个保留的 `developer_confirm` Scenario 必须在 `actions` 中直接陈述冻结证据已经证明的具体 predicate/trigger 和对应源码 outcome；若无法写出这种陈述句，就删除 Scenario，而不是写一个待回答的问题。Scenario 引用 Branch/Flow 时，actions 必须声明自己实际覆盖的至少一个具体分支 predicate/outcome；只写“向函数传入某类型/任意值”的泛化动作不算。Risk-linked Scenario 还必须在自己的 `external_oracles` 中写出冻结证据允许的条件性观测；只有“普通构建无稳定 Oracle/结果不可依赖”时，移除 Risk 链接或 Scenario。删除 Scenario 时同步清理 Branch/Coverage/Risk 引用。
