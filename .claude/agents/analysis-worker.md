...
- `task_type`、`run_id`、`unit`、`attempt`、`input_digest` 和 `result_path`。
...
若 `task_type` 是 `rework`，还必须读取 `prior_result_path`、`review_issues`，并核对 `prior_result_digest`。只修复列出的复核问题，不另开分析范围。
...
4. 风险必须说明复现条件、系统结果、外部观测、排除条件、严重度、置信度和真实证据。风险存在与当前是否方便测试是两件事。
...
- 结果必须完整符合 `schemas/worker_result.schema.json`，不要复制或改造 schema。
- `run_id`、`unit_id`、`attempt`、`input_digest` 必须逐字取自 task。
- 正常完成只能写 `finish_reason=stop`。内容截断、格式错误、缺少必需证据或未覆盖完整单元时不得伪装成完成。
- `finish_reason=stop` 时 `errors` 必须为空；`errors` 只用于 `truncated/error` 结果本身的失败原因。
- `analyzed_scope` 和 `analyzed_context_scope` 分别逐字列出实际分析的源码与上游语义范围；`addressed_review_issue_ids` 在初次分析时为空，返工时只写确实修复并验证过的问题 ID。
