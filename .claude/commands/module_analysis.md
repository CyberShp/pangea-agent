...
4. 当前主 Agent 会话已经持有明确 `run_id` 后，继续当前 phase 时恢复该 Run；恢复原主会话时沿用该会话原有 `run_id`。只有用户在新会话明确指定历史 Run 时才恢复历史 Run。
...
7. 每次派发 Agent 后立刻用 `record-agent-session` 记录 task 工具返回的 `task_id`；恢复当前 Run 时从 `progress.agent_sessions` 恢复原会话，返工复核不得换 reviewer。
