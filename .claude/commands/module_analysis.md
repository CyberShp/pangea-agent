...
6. 当前 Agent 回合通过提交校验后，主 Agent 立即按 `after_completion=resume_run` 恢复同一 `run_id` 和 `data_root`，不得把 schema FAIL 的结果留给后续 review 处理。
7. 只对 `dispatch_agent` 立即用 `record-agent-session` 记录返回的 `task_id`；`continue_agent` 恢复 action 指定的原会话，不重复记录，返工复核不得换 reviewer。
