...
关键判断用独立节点表达；不同条件走向实际处理或结果节点，汇合时再引用同一后继。若两条路径条件不同但node_ids完全相同，核对是否漏掉判断后的处理差异，不靠改标题表达真假分支。普通顺序连线的condition留空，只有实际守卫或选择条件才填写，不能把“下一步”当作条件分支。调用者不在冻结范围时，区分已证实的函数内行为与待确认的业务调度顺序，不把使用建议写成已验证调用链。
