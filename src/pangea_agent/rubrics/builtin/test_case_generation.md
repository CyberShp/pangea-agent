...
- developer-confirm Scenario 写入前，逐个 Scenario 单独从自己的 actions 原文抄出具体 predicate/trigger 与对应源码 outcome；两者必须存在于这个 Scenario 自己的 actions 数组，不能从 title、preconditions、external_oracles 或兄弟 Scenario 借用。action 仍在询问、等待确认如何操作时，应把缺口留在 business_entry/preconditions/reason，并删除该 Scenario 或改写真正由源码证明的内部操作。受支持入口未确认时，action 不得声称通过产品入口或公开 API 执行。删除 Scenario 后重算 Branch/Coverage/Risk 引用。若保留关联 Risk 的 Scenario，external_oracles 还必须有冻结证据允许的条件性观测，不能只写普通构建无稳定 Oracle；按 usual arithmetic conversions 后的真实有符号运算类型使用对应 `TYPE_MAX`，单点边界不能扩写成“其他极大值”“附近值”或更宽输入域；只有运算类型确为 `int` 的 `value + 1` 才使用 `value == INT_MAX`。Scenario 同时填写 `branch_ids` 时，还必须由自身 action 与 Oracle 覆盖自己声明的分支 outcome/结果。单个 Scenario 不强制同时覆盖 true/false，否则拆分或移除该引用。
