...
资产提供分析线索；引用资产不等于证明当前实现存在该问题。在已有材料消费记录和风险说明中完成以下推导，不新增索引或机械语义门禁：

- 区分资产中的需求约束、设计意图、历史故障和当前源码事实；保留资产 ID、revision 与具体读取位置。
- 对资产引出的风险，核对当前源码是否仍有同一操作、可达触发条件和传播路径，并指出源码位置。历史实现已被替换或保护条件已排除机理时，将其作为回归关注点，不写成当前缺陷。
- 主动寻找能推翻结论的反例、保护分支或等价行为。声称条件或补丁变化会造成故障时，比较变化前后在触发输入上的返回值、状态与外部结果；只有路径不同而结果相同，不能据此声称失效。
- 数值风险按当前运算及语言规则核对定义域和边界，不把其他运算的溢出条件直接套用。平台、调用约束或历史机理缺乏证据时，明确待确认条件；不得借无证据的特殊平台假设把问题升级为已确认风险。
- 语言标准版本、整数表示和目标实现约束分别说明依据，不能把常见机器特性或较新标准要求写成所有版本的保证。未确定的表示或边界值保留条件式结论；对一个表达式的证明不得扩大为该运算符在所有操作数下都安全。

结论由 Agent/Reviewer 判断。Step 05 和 Step 08 按 `references/worker-judge-protocol.md` 执行完整契约、适用范围、传播路径、反事实和根因归并的候选裁决，保留驳回依据；不为满足风险数量而补造缺陷。
...
- 场景候选是否来自多源演绎；
- SFMEA 是否有源码机制；
- 资产引出的风险是否把历史事实或设计意图误当成当前实现，触发条件与传播路径是否有当前源码依据；
- Producer 所述故障是否被保护条件、反例或等价输出推翻，数值推导是否符合实际运算的语言规则；
- 黑盒步骤是否可以外部执行；
- Oracle 是否独立；
- 测试是否过少、重复或泛化；
...
即使由同一 Agent 执行复核，也必须重新读取被引用的资产片段和当前源码，先尝试反证再决定保留、降为待确认或排除；不能以此前写出的风险说明或流程校验通过代替复核。该模式应如实称为自审，不宣称独立第二 Agent 审核。
...
- `READY`：兼容字段，仅表示 run_guard 的流程与交付结构检查通过，不是 Python 给出的语义认可。实际审查方式、交付完整性和 Reviewer 的 `semantic_verdict` 分开呈现
