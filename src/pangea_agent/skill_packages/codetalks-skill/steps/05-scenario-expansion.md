# Step 05：多源场景增殖和风险解释

## 目标

从实现事实演绎场景，并向测试人员解释每个风险为什么存在。

## 必须输出

- `活文档/13-场景候选池与推导说明.md`
- `活文档/14-风险点清单与因果说明.md`
- Step 05 Coverage Gate

## 场景候选池

先按八类驱动源分别总结发现，再列候选：

- 分支；
- 状态；
- 资源和不变量；
- 数值/N/2N/翻转；
- 并发；
- 异常传播；
- 需求/协议/安全/配置；
- 覆盖率和历史证据。

每个候选必须解释从哪段实现事实推导出来，不能只列名称。

## 候选成立与归并

先读取 `references/worker-judge-protocol.md` 的“候选裁决”部分。在候选池的原条目内完成契约、范围、传播、反事实和根因归并五项判断，再决定是否进入风险清单。
候选池保留被排除和被合并的条目及依据；只有成立的当前实现风险进入 `risks` 投影。覆盖缺口关联补测用例，适用限制进入范围说明，不占风险数量。

## 风险点

每个风险用因果叙述：

```text
什么条件发生
→ 代码内部哪里失效
→ 状态/资源/数据留下什么问题
→ 为什么当前操作可能仍看似正常
→ 什么时候对外暴露
→ 黑盒如何证明
```

风险标题必须包含在本次 Run 内稳定且唯一的 `risk_id`。写入阶段工作台投影时，沿用同一个 ID，并使用统一字段：

```json
{
  "risk_id": "R-001",
  "severity": "High",
  "severity_source": "workbench_projection",
  "narrative": "风险概要",
  "trigger": "什么条件发生",
  "system_result": "代码内部哪里失效",
  "residual_effect": "状态、资源或数据留下什么问题",
  "apparent_normality": "为什么当前操作可能仍看似正常",
  "external_observation": "什么时候对外暴露",
  "blackbox_proof": "黑盒如何证明",
  "source_section": "活文档/14-风险点清单与因果说明.md#R-001"
}
```

`severity` 只使用 `Critical`、`High`、`Medium`、`Low`。等级由 Agent 根据分析写出；若尚未完成分级则省略该字段，不得用默认等级补齐。`severity_source` 记录实际来源。SFMEA 失效模式与风险的关系必须显式写出，不能按编号或标题相似度推断。

阶段投影用于工作台及时展示。字段暂缺只表示草稿尚未补齐，不得因此把 Run 判为失败；后续步骤应在保留相同 `risk_id` 的前提下补全。


## 目录限制

本步骤只写入 `活文档/` 和 `内部索引/`，不得写入 `正式输出/`。
