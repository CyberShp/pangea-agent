...
.detail-body code{{overflow-wrap:anywhere;word-break:break-word}}.table-wrap code{{white-space:nowrap;word-break:normal}}.table-wrap th:first-child,.table-wrap td:first-child{{white-space:nowrap}}
</style>
</head>
<body id="top">
...
def render_html_report(state: Mapping[str, Any]) -> str:
    ...
    for index, flow in enumerate(_items(flows), 1):
        ...
        diagram = flow.get("mermaid") or flow.get("diagram")
        diagram_html = ""
        if diagram:
            diagram_html = f'<p class="mermaid-note">离线报告不内嵌 Mermaid 运行库；以下源码保持可读，可复制到受信任的 Mermaid 工具渲染。</p><pre>{html.escape(str(diagram))}</pre>'
    ...
    body.append(f'<section id="incomplete"><a class="back" href="#top">回到顶部</a><h2>7. 不完整项与未解析证据</h2>{incomplete_html}</section>')
    ...
    body.append(f'<section id="quality"><a class="back" href="#top">回到顶部</a><h2>8. 质量门禁</h2><p class="summary {final_tone}">{_escape(_quality_summary(state, incomplete))}</p><h3>已完成检查</h3>{checks_html}{unresolved_html}<p><span class="badge {final_tone}">{completeness}</span></p></section>')
