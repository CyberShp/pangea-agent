...
- 机械门禁只能保护下游真实需要的数据完整性和可追溯性，例如 JSON 可解析、必填字段、固定枚举、真实 ID、显式路径归属和引用存在。不得根据关键词、行号重叠、数量、措辞、finding category 或测试用例内容判断分析质量，也不得增加测试用例语义门禁。
...
- 新增门禁的验收必须同时包含：合法语义结果能够通过、真实结构错误能够被阻断、同一 Agent 修正后能够继续，以及失败不会被报告为成功。只证明“能拦住错误”不算完成。
